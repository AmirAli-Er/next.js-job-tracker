...
// it will be able to be called from Client side components while intract with database(intefere directrly with database)
export  async function createJobApplication(data:JobApplication){
  ...
  //   -1 means descending order, so the application with the largest order comes first
    .sort({ order: -1 })
    // Return only the order field
    .select("order")
    // Convert the Mongoose document into a plain JavaScript object
    // as =>  Tell TypeScript the expected result type
